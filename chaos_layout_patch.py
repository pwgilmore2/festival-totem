...
.xyWrap{margin-top:4px;padding-top:3px;border-top:1px solid #ffffff14}
...
<div class="chaosGroup"><div class="chaosGroupLabel">Heavy / Impact <span class="chaosGroupHint">drops, bass hits, breakdown carnage</span></div><div class="chaosFxGrid">
...
<button class="fxBass" onpointerdown="safeHoldStart(event,'bassjostle')" onpointermove="safeHoldMove(event)" onpointerup="safeHoldEnd(event)" onpointercancel="safeHoldEnd(event)">🔊<br>BASS JOSTLE</button>
...
<button class="fxPixelMelt" onpointerdown="safeHoldStart(event,'pixelmelt')" onpointermove="safeHoldMove(event)" onpointerup="safeHoldEnd(event)" onpointercancel="safeHoldEnd(event)">🔥<br>PIXEL MELT</button>
...
<div class="chaosGroup"><div class="chaosGroupLabel">Chill / Flow <span class="chaosGroupHint">builds, melodic sections, trancey stuff</span></div><div class="chaosFxGrid">
...
<div class="xyWrap"><div class="sh"><span>TRUE CHAOS PAD</span><span class="tiny">drag + move fast</span></div><div id="chaosXY" class="xyPad" onpointerdown="xyStart(event)" onpointermove="xyMove(event)" onpointerup="xyEnd(event)" onpointercancel="xyEnd(event)"><div id="xyDot" class="xyDot"></div></div></div>
...
function safeHoldStart(e,kind){
  ...
  safeHold={id:e.pointerId,kind,el,x:e.clientX,y:e.clientY,fired:false,timer:null};
...
function safeHoldEnd(e){
  if(!safeHold||safeHold.id!==e.pointerId)return;
  clearTimeout(safeHold.timer);let fired=safeHold.fired,el=safeHold.el;
  el.classList.remove('holdPending','pressed');safeHold=null;
  if(fired)cmd('guest_stop');
}
</script>
'''

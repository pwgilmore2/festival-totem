...
.textCard{background:radial-gradient(circle at 10% 0%,#6c4cff22,transparent 35%),radial-gradient(circle at 100% 10%,#00c8ff18,transparent 38%),#ffffff12}.textShow{width:100%;min-height:58px;font-size:18px;background:linear-gradient(135deg,#6c4cff,#00b8ff)}.textEffects{display:grid;grid-template-columns:repeat(2,1fr);gap:8px}.textEffects button.active{background:linear-gradient(135deg,#6c4cff,#00b8ff)}.colorRow{display:grid;grid-template-columns:1fr 74px;gap:8px}.colorRow input[type=color]{width:74px;height:46px;border:0;border-radius:10px;background:#202029;padding:5px}.textBgBox{margin-top:14px;padding-top:12px;border-top:1px solid #ffffff14}
...
@media(max-width:520px){.layerGrid,.textGrid{grid-template-columns:1fr}.sceneGrid{grid-template-columns:repeat(2,1fr)}.tabs button{min-width:86px}.guestGrid button{min-height:72px}}
</style>
"""
...
SCENE_CARD = r"""
<div class="card"><h2>Performance Scenes</h2><div class="sectionHint">One tap changes slideshow pace, transition feel, reactive preset, intensity, and beat sync together.</div><div id="sceneButtons" class="sceneGrid"></div></div>
"""
...
<div class="sectionHint" id="beatSyncHint">When enabled, the timer becomes the minimum display time, then the slideshow waits for the next detected beat. It falls back automatically if no beat arrives.</div>
...
<div class="colorRow" style="margin-top:10px"><select id="textColorMode" class="selectDark" onchange="textChanged()"></select><input id="textColor" type="color" value="#ffffff" oninput="textChanged()"></div>
...
<div class="textEffects"><button id="textGlow" onclick="toggleTextFx('glow')">✨ Glow</button><button id="textWave" onclick="toggleTextFx('wave')">〰 Wave</button><button id="textGlitch" onclick="toggleTextFx('glitch')">⚡ Glitch</button><button id="textBeatPulse" onclick="toggleTextFx('beat_pulse')">♫ Beat Pulse</button></div>
<div class="textBgBox"><div class="sh"><span>Background</span><span class="tiny">GIF brightness capped at 55%</span></div><select id="textBackground" class="selectDark" onchange="textChanged()"></select><div class="slider"><div class="sh"><span>Background brightness</span><span id="textBgValue">28%</span></div><input id="textBgBrightness" type="range" min=".05" max=".55" step=".05" value=".28" oninput="pct('textBgValue',this.value);textChanged()"></div><button id="textBackplate" style="width:100%" onclick="toggleTextFx('backplate')">▰ Text Backplate</button><div class="sectionHint" style="margin-top:8px">Dimmed GIF keeps the original colors. Tinted GIF converts the animation toward your selected text color. Backplate darkens only the pixels immediately behind the message.</div></div>
<button class="textShow" style="margin-top:12px" onclick="showText()">SHOW TEXT</button>
<div class="sectionHint" style="margin-top:10px">Rainbow cycles each character. Audio color follows the current music bands. Beat Pulse works whenever phone audio is running.</div></div></section>
"""
...
GUEST_SECTION = r"""
<section id="guest" class="view"><div class="card chaosCard"><h2 class="chaosTitle">CHAOS PAD</h2><div class="chaosSub">Smash buttons. Hold chaos. Saved setup stays untouched.</div><div class="guestGrid" style="margin-top:12px">
<button class="padBoom" onclick="guestAction('boom')">💥<br>BOOM</button><button class="padGlitch" onclick="guestAction('glitch')">⚡<br>GLITCH</button><button class="padSpark" onclick="guestAction('spark')">✨<br>SPARK STORM</button><button class="padRainbow" onclick="guestAction('rainbow')">🌈<br>RAINBOW</button><button class="padWarp" onclick="guestTransition('Ripple')">🌀<br>WARP → NEXT</button><button class="padMelt" onclick="guestTransition('Melt')">🫠<br>MELT → NEXT</button><button class="padRandom" onclick="guestAction('random')">🎲<br>RANDOM GIF</button><button class="padNext" onclick="guestAction('next')">➡️<br>NEXT GIF</button><button class="padParty" onclick="guestParty()">🎉<br>PARTY MODE</button><button class="padWarp" onclick="guestTransition('Zoom')">🛸<br>ZOOM → NEXT</button><button class="guestBig" onpointerdown="chaosStart(event)" onpointerup="chaosEnd(event)" onpointercancel="chaosEnd(event)" oncontextmenu="event.preventDefault()">HOLD FOR CHAOS</button>
...
let layerSignature='',sceneSignature='',textTimer=null,textFxLocal={glow:false,wave:false,glitch:false,beat_pulse:true,backplate:true};
...
function applyScene(name){cmd('performance_scene',name)}
...
def enhanced_html(source):
    ...
    html=html.replace(effects,'',1).replace(playback,'',1).replace(slideshow,LIBRARY_CARD+SCENE_CARD+SHOW_CARD,1).replace(library_section,'',1)
    html=html.replace('<section id="edit" class="view">',TEXT_SECTION+'\n'+GUEST_SECTION+'\n<section id="edit" class="view">'+DISPLAY_CARD,1)
    html=html.replace('<div id="reactiveLayerMount"></div>',LAYER_BLOCK,1).replace('effectRender();','')

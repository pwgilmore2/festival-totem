...
function syncPerformanceUI(){if(typeof state==='undefined')return;renderLayerControls();let tr=state.transition||{},select=document.getElementById('transitionKind');if(select){let options=state.transitions||[],sig=options.join('|');if(select.dataset.sig!==sig){select.dataset.sig=sig;select.innerHTML='';options.forEach(n=>{let o=document.createElement('option');o.value=n;o.textContent=n;select.appendChild(o)})}if(document.activeElement!==select&&tr.kind)select.value=tr.kind}let td=document.getElementById('transitionDuration');if(td&&document.activeElement!==td&&tr.duration!=null)td.value=tr.duration;if(window.transitionDurationValue&&tr.duration!=null)transitionDurationValue.textContent=parseFloat(tr.duration).toFixed(1)+'s';if(window.randomTransition){randomTransition.textContent='Random: '+(tr.random?'On':'Off');randomTransition.classList.toggle('active',!!tr.random)}}
...
def enhanced_html(source):
    ...
    old_tabs='<button id="tabLive" class="active" onclick="view(\'live\')">Live</button><button id="tabLibrary" onclick="view(\'library\')">Library</button><button id="tabAudio" onclick="view(\'audio\')">Audio</button><button id="tabEdit" onclick="view(\'edit\')">Edit</button>'
    new_tabs='<button id="tabLive" class="active" onclick="view(\'live\')">Library</button><button id="tabAudio" onclick="view(\'audio\')">Audio</button><button id="tabGuest" onclick="view(\'guest\')">Chaos Pad</button><button id="tabEdit" onclick="view(\'edit\')">Setup</button>'
    html=html.replace(old_tabs,new_tabs,1)
    html=html.replace('["live","library","audio","edit"]','["live","audio","guest","edit"]')

    effects='<div class="card"><h2>Effects</h2><div class="g3" id="effects"></div></div>'

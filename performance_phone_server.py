...
.transitionGrid,.layerGrid,.guestGrid,.sceneGrid{display:grid;gap:8px}.transitionGrid{grid-template-columns:repeat(2,1fr)}.layerGrid{grid-template-columns:1fr 1fr}.guestGrid{grid-template-columns:repeat(2,1fr)}.sceneGrid{grid-template-columns:repeat(4,1fr)}
.sceneGrid button{min-height:52px}.sceneGrid button.active{box-shadow:0 0 0 2px #ffffff55 inset,0 0 22px #7063d766}.selectDark{width:100%;background:#202029;color:#fff;border:1px solid #444456;border-radius:10px;padding:11px;font-size:16px}.tiny{font-size:11px;opacity:.65}.sectionHint{font-size:12px;opacity:.7;line-height:1.4;margin:-3px 0 10px}
...
@media(max-width:520px){.layerGrid{grid-template-columns:1fr}.sceneGrid{grid-template-columns:repeat(2,1fr)}.tabs button{min-width:86px}.guestGrid button{min-height:72px}}
</style>
"""
...
let layerSignature='',sceneSignature='';
...
def enhanced_html(source):
    ...
    html=html.replace(old_tabs,new_tabs,1).replace('["live","library","audio","edit"]','["live","audio","guest","edit"]')

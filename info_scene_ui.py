...
<div class="card infoSceneCard"><h2>Clock and Weather</h2><div class="muted">Time comes from your phone in Arkansas time. Weather refreshes only when you ask; the last result stays on screen offline.</div><button onclick="syncTotemClock()">Sync time from phone</button>
...
<div class="infoSceneGrid"><button onclick="refreshTotemWeather(true)">Use phone location</button><button onclick="refreshTotemWeather(false)">Festival location</button></div><div class="muted" id="weatherStatus">Phone location needs browser location permission and HTTPS. Festival location works without GPS.</div>
...
_JS = r'''<script>
(function(){
 const SCHEDULE_KEY='festivalTotem.setTimes.v2',OLD_SCHEDULE_KEY='festivalTotem.setTimes.v1',WEATHER_KEY='festivalTotem.weather.v1';
 ...
 setInterval(()=>{if(typeof state!=='undefined'&&['Clock','Weather','Set Times'].includes(state?.info_scene))syncTotemClock()},60000);
 try{

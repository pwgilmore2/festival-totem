...
.vibeStart{background:linear-gradient(125deg,#ee467c,#8152ef 52%,#208cdd);border:1px solid #ffffff58;letter-spacing:.03em}.vibeTransport button:first-child{background:linear-gradient(130deg,#3b568b,#65488c)}.vibeTransport button:last-child{background:linear-gradient(130deg,#705088,#3b7294)}.tabs button.performancePrimary:nth-of-type(4n+1){background:linear-gradient(135deg,#344c77,#493c82)}.tabs button.performancePrimary:nth-of-type(4n+2){background:linear-gradient(135deg,#70507b,#934a70)}.tabs button.performancePrimary:nth-of-type(4n+3){background:linear-gradient(135deg,#3b7383,#3d5392)}.tabs button.performancePrimary:nth-of-type(4n){background:linear-gradient(135deg,#8b516e,#69439c)}.tabs .managementTab{background:linear-gradient(135deg,#57526b,#3d465e)!important}.tabs .managementTab.active{background:#343442!important}.vibeTransport button,.sceneChooser button,.vibePresetGrid button,.manageSwitcher button{border:1px solid #ffffff2b;box-shadow:inset 0 1px #ffffff19,0 4px 12px #0003;touch-action:manipulation}
...
.vibePresetGrid button{min-height:58px;font-size:13px;text-transform:uppercase;letter-spacing:.075em;overflow-wrap:anywhere;background:linear-gradient(140deg,var(--preset-a,#6652b9),var(--preset-b,#296992))}.vibePresetGrid button.active{color:#fff}.vibePresetGrid button:active,.sceneChooser button:active{transform:scale(.97)}.overlayAudio button:nth-child(1){background:linear-gradient(140deg,#4c5169,#585277)}.overlayAudio button:nth-child(2){background:linear-gradient(140deg,#356f86,#4a57a0)}.overlayAudio button:nth-child(3){background:linear-gradient(140deg,#933f81,#7050ad)}
...
.manageSwitcher button:nth-child(1){background:linear-gradient(140deg,#355f88,#544996)}.manageSwitcher button:nth-child(2){background:linear-gradient(140deg,#745184,#a04e75)}.manageSwitcher button:nth-child(3){background:linear-gradient(140deg,#397b77,#5357a3)}.manageSwitcher button.active{background:#343442!important}.manageSwitcher button.active:after{content:"";display:inline-block;width:7px;height:7px;margin-left:6px;background:#65f2b6;border-radius:50%}
...
(function(){
 ...
 function scrollManage(kind){const el=document.querySelector('.manageFold[data-manage-group="'+kind+'"]')||tab('panelCard');if(el)setTimeout(()=>el.scrollIntoView({behavior:'smooth',block:'start'}),40)}
 window.openManage=function(kind){if(!['library','edit','setup'].includes(kind))kind='library';currentManage=kind;setManageMode(true);const target=(kind==='library')?'live':'edit';_performanceBaseView(target);if(window.selectManageGroup)selectManageGroup(kind);document.querySelectorAll('.tabs button').forEach(b=>b.classList.remove('active'));const m=tab('tabManage');if(m)m.classList.add('active');markManage(kind);scrollManage(kind)}
 ...
 window.renderMyVibes=function(){
   ...
   if(signature!==renderedVibes){
     renderedVibes=signature;const colors=[['#bb467d','#744fc5'],['#308a9b','#4461bc'],['#b46b3e','#974a83'],['#448f6b','#3b699e'],['#9655ac','#347fac'],['#9b5973','#5c5eb0']];boxes.forEach(box=>{box.innerHTML='';presets.forEach(p=>{const b=document.createElement('button');b.textContent=p.name;b.classList.toggle('active',p.id===selectedVibeId);const hash=[...p.id].reduce((n,c)=>((n*31+c.charCodeAt(0))>>>0),0),pair=colors[hash%colors.length];b.style.setProperty('--preset-a',pair[0]);b.style.setProperty('--preset-b',pair[1]);b.onclick=()=>applyMyVibe(p.id);box.appendChild(b)});if(!presets.length)box.innerHTML='<div class="muted" style="grid-column:1/-1">No presets. Name the current settings and tap Add New.</div>'})
   }

...
(function(){
 ...
 window.applyBatchTag=async function(){const input=document.getElementById('batchTagInput'),tag=(input?.value||'').trim();if(!tag||!batchSelected.size)return;const originalTarget=state.target||'both',p=state.panels||{},front=p.front?.image_index_zero,back=p.back?.image_index_zero;for(const index of [...batchSelected]){const item=(state.library||[]).find(x=>x.index===index);const tags=[...(item?.tags||[])];if(!tags.includes(tag))tags.push(tag);await cmd('set_target','both');await cmd('select_image',index);await delay(18);await cmd('set_tags',tags);await delay(18)}if(Number.isInteger(front)){await cmd('set_target','front');await cmd('select_image',front)}if(Number.isInteger(back)){await cmd('set_target','back');await cmd('select_image',back)}await cmd('set_target',originalTarget);batchSelected.clear();syncBatchUI()}
